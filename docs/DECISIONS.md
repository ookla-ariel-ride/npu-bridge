...
## D104: the chunk8-leftovers wave (2026-09-21)
The chunk8-leftovers wave closed issues #24, #27, #28, #17 and #33 in five code tickets and their reviews. It ran on `wave/chunk8-leftovers`, cut from `main` at `66e309b`; the final code tip was `949ed25`, with 996 tests. The last clean smoke on 2026-09-21 passed all steps, with no skipped steps, seven informational steps and no first-generation RPC retry. The full smoke evidence and the wave's process record are in the handoff.

**#24. The queue-depth gate was kept.** SQ-42's production change remembered a cancellation between `TryWrite` and `MarkEnteredQueue` and released the live depth at entry rather than waiting for dequeue. SQ-48's review established that the old code released exactly once through dequeue, so the old behavior was a bounded over-count for up to one generation, a deviation from D87's “whichever comes first”, never a leak. The candidate releases exactly once by construction: one call site is behind a never-reset `CompareExchange` on `_queueDepthClaimed`; 25,000 barrier-released races produced zero double fires. The new flag is a second Dekker pair on the same `Interlocked` discipline. Reverting the gate and teaching the test the dequeue was rejected.

**Addendum to D92:** The same publish-before-arm shape has a third signal: a caller's cancel can fire between TryWrite and MarkEnteredQueue, and the callback's LeaveQueueIfNeeded then returns at the unarmed guard. Unlike the MarkDequeued case this was never a leak, the job is still in the channel and the worker's later dequeue is a guaranteed backstop, so the counter self-corrects, but it deferred the decrement to the dequeue and so broke D87's "whichever comes first" for up to one generation. _cancelledWhileQueued is remembered with Interlocked.Exchange and re-checked by MarkEnteredQueue alongside _dequeued, making the arm/leave state a three-signal gate on the same Dekker discipline. Exactly-once is still the CompareExchange on _queueDepthClaimed and not the flags.

**#27. The stale-timeout guard was deliberately left without a dedicated test.** SQ-49 showed that the guard in `WaitForDeltaAsync` is decided by the thread-pool scheduler: the test failed 25/25 times at `DOTNET_PROCESSOR_COUNT=1` and failed 0.5 to 10 percent under pool load on a 12-core host. No `FakeTimeProvider` arrangement reached the guard because completing the source task resolved `Task.WaitAsync`'s cancellation promise before its timer fired, with 0/1000 and 0/5000 trials in the two arrangements. The old FUTURE premise was withdrawn. An injectable timeout factory or a `Task.WhenAny` over `time.Delay` was rejected as a production seam for a sub-microsecond race; keeping the test probabilistic was rejected because the result was environmental. `DrainBufferedAsync` still computes its cadence from `Stopwatch` while its waits use `TimeProvider`; that is harmless under `TimeProvider.System` and is deferred. Item 4, the sibling truncation window, also stays untested without a new fake gate.

**#28. The drain warning took four rounds.** SQ-44 (candidate `2d368fc`, integrated at `04d07de`) added periodic warnings while a cancelled generation drained, with no timeout and no dispose, preserving D51. SQ-53 (candidate `2a77bca`, integrated at `5d7a0b0`) fixed the binder range at 86,400 seconds, carried the JSON client-abort through the drain loop, changed the test interval to 60 seconds, added the completions-stream test and the `--drain-warning-seconds` flag and help text. SQ-55 (candidate `822b319`, integrated at `94b6014`) tried to fix a streamed abort that produced no further delta, but the reader loop still read with `CancellationToken.None`; the abort was noticed only when a write threw, and the test passed through `max_tokens = 1`. The final hard-route fix, SQ-56 (candidate `0b7897d`, integrated as `949ed25`), made both reader loops use the aborted token, retained the single drain and dispose, added the completed-task fast path, and removed the false test shortcut. The first three rounds missed the scenario because the reader loop's `CancellationToken.None` read, which its own comment justified, left cancellation invisible until a write threw. There is deliberately no timeout: disposing a context while generation may still write would restore D51's use-after-dispose race.

SQ-52's findings landed in that follow-up: the binder max fixed the `Task.Delay` range, the JSON path entered the loop, the tests used a 60-second interval, completions-stream coverage was added, the CLI flag and help text were added, and endpoint labels were made explicit. Its F7 finding, no keep-alive during a drain, went to FUTURE. The streamed shapes' pre-Acquire silent wait also remains deferred. SQ-58 cleared the hard-route behavior and raised F1: after `TryComplete()`, the closure can still run `attemptLease.Dispose()` on the thrown path and the scheduler settles the task, so the plain await can observe that tail without a warning. That finding was accepted as evidence and rejected as a blocker. The generation has ended in that window, nothing observed showed `Dispose` blocking, and restoring warning parameters at seven call sites would add plumbing that the review had just identified as warning nothing. The claim is narrowed: the streamed abort is warned from the reader loop's exit and in the finally; the closure's tail after channel completion is not.

**#17. Option 1 was selected.** `AionBackend` keeps `Cancellation` withheld until measured, and the smoke expects `[]` for Aion.

**#33. The empty-fence instruction was kept.** On build 29667, `tool-probe.ps1 -Include MultiStep` ran ten invocations per build with a fresh conversation each: the wave tip `fab9e97` answered in prose on all 10/10 turn-2 requests and hit the cache on every round trip; control `main` at `66e309b` also answered in prose on 10/10. The empty fence observed on 2026-09-13 did not reproduce in twenty round trips. The sentence stayed, the parser does not strip an empty fence, and clients are told to re-ask when a reply consists only of that fence. Option 1, stripping it and returning empty content, was rejected because it hides what the model did and makes the reply indistinguishable from a genuine empty answer.

The wave also recorded three process lessons. A review filed after integration binds to the integrated hash rather than the candidate, so reviews should bind before integration or explicitly review the hash. The board refuses rework on a review-bound candidate; a rejected candidate is superseded by a fresh repair ticket, as SQ-43 became SQ-51. A negative control must stay inside a ticket's declared files; SQ-50 met the scope wall on `GenerationScheduler.cs`.

Two dissenting recommendations were retained. `DrainWarningSeconds` remained a user-facing option despite the recommendation to remove the speculative knob: an operator watching a wedged runtime is its consumer, and the flag carries the environment value across the Phi Silica self-relaunch. SQ-58 F1 was recorded and deferred rather than fixed: the generation had ended, no blocking dispose was observed, and the proposed repair would re-add three parameters at seven call sites to warn during a dispose.
